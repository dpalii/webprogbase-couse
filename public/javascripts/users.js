...
let entrySearchword = ''
Promise.all([
    fetch(`/api/v1/users?limit=${entryLimit}&offset=${entryOffset}&searchword=${entrySearchword}`, { headers: { Authorization: `Bearer ${jwt}` }}).then(x => x.json()),
...
.then(([data, template, pagination]) => {
    if (data.data.users) {
        ...
        usersEl.innerHTML = renderedHtmlStr;
...
function getPageData(limit, offset, searchword) {
    Promise.all([
        fetch(`/api/v1/users?limit=${limit}&offset=${offset}&searchword=${searchword}`, { headers: { Authorization: `Bearer ${jwt}` }}).then(x => x.json()),
        fetch('/templates/users.mst').then(x => x.text()),
    ])
    .then(([data, template]) => {
        const renderedHtmlStr = Mustache.render(template, data.data);
